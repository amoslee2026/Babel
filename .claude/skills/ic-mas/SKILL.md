---
name: ic-mas
description: "生成芯片功能模块微架构文档（MAS、FSM、数据通路、验证计划、DFT）。Generate microarchitecture specification for chiplet modules. Trigger: /ic-mas, 微架构文档, MAS, FSM, datapath, verification plan, DFT"
user-invocable: true
self-adaptive: true
version: "1.0.0"
arguments:
  ...
  - name: finetune
    ...
    required: false
handoffs:
  - target: ic.spec-review
    condition: "MAS documents completed"
    session_note: "建议在新会话中执行 ic.spec-review"
evolution:
  enabled: true
  trigger: on_failure
  max_attempts: 3
...
初始化目录结构：`.skills_local/ic-mas/`，并收集项目上下文信息。
...
2. 加载配置文件（MANDATORY）
   ...
   with open(".skills_local/ic-mas/project_context.json") as f:
       CONTEXT = json.load(f)
...
```
spec_arch/ ──→ [ic-mas] ──→ spec_mas/ ──→ ic.spec-review ──→ ic.impl
                  OUTPUT_DIR
```
...
1. **Spec 先行铁律**：`spec_arch/` 中有效文档少于 2 份 → 拒绝执行，返回 ic-arch 阶段
...
TEMPLATE_DIR      = ~/.claude/skills/ic-mas/templates
PROGRESS_DIR      = {{ OUTPUT_DIR }}/.progress
CHECKPOINT_DIR    = {{ OUTPUT_DIR }}/.checkpoint
SCRIPT_DIR        = ~/.claude/scripts
SKILL_FILE        = ~/.claude/skills/ic-mas/SKILL.md
```
...
| "先标 status: complete，内容后续完善" | 错误的 complete 标记会让 ic.spec-review 跳过实际未完成模块，掩盖质量问题 |
...
| ic.spec-review 不可用 | 使用阶段 5 `plan.md` 内联质量检查清单代替，结果写入 `.checkpoint/manual_check.md` |
...
> 以下条件全部满足才可声明 ic-mas 完成，并触发 ic.spec-review handoff。缺一不可。
...
- [ ] ic.spec-review 未报告 CRITICAL 级别问题（或已修复并记录）

**禁止在上述条件未满足时触发 ic.impl handoff。**
