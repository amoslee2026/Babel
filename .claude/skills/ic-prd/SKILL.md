---
name: ic-prd
description: "生成芯片/IP 产品需求文档（PRD），从市场需求和技术约束输出专业 PRD 规范。Generate Product Requirements Document for chip/IP design projects. Trigger: /ic-prd, PRD生成, 产品需求文档, 芯片立项, IP需求, chiplet specification"
user-invocable: true
self-adaptive: true
self-evolving: true
version: "1.0.0"
...
handoffs:
  - target: ic-arch
    ...
    session_note: "PRD 完成后可进入架构设计阶段，调用 ic-arch"
    transfer_files:
      - "{{OUTPUT_DIR}}/*.md"
evolution:
  enabled: true
...
1. Self-Adaptive 初始化 + 项目上下文收集
   ...
   ```
   成功标志：
   - `.skills_local/ic-prd/local.json` 存在
   - `.skills_local/ic-prd/project_context.json` 存在
   - `.skills_local/ic-prd/paths.json` 存在

2. 加载配置文件
   ```python
   import json
   
   with open(".skills_local/ic-prd/project_context.json") as f:
       CONTEXT = json.load(f)
...
```
市场需求/idea ──→ [ic-prd] ──→ prd/ ──→ ic-arch ──→ spec_arch/ ──→ ic-mas ──→ spec_mas/
                      OUTPUT_DIR
```
...
TEMPLATE_DIR      = ~/.claude/skills/ic-prd/templates
REFERENCE_DIR     = ~/.claude/skills/ic-prd/references
SCRIPT_DIR        = ~/.claude/scripts
SKILL_FILE        = ~/.claude/skills/ic-prd/SKILL.md
...
| Agent 的借口 | 为什么错 |
|-------------|---------|
| "需求文档不完整，但可以先生成 PRD 框架" | PRD 空框架比没有更危险——下游 ic-arch 会基于错误假设展开，修正成本以人月计 |
...
| "性能指标暂时用 TBD，后续再定" | TBD 指标会传播到 ic-arch 的 timing budget，导致整个设计基础不稳 |
...
> 以下条件全部满足才可声明 PRD 完成，并触发 ic-arch handoff。缺一不可。
...
**禁止在上述条件未满足时声明 PRD 完成或触发 ic-arch。**
