...
arguments:
  ...
  - name: finetune
    ...
    default: "false"
handoffs:
  - target: ic.synthesis-check
    condition: "Architecture spec completed"
    session_note: "架构完成后可进入综合检查流程"
    transfer_files:
      - "{{OUTPUT_DIR}}/*.md"
evolution:
  enabled: true
...
初始化目录结构 `.skills_local/ic-arch/`，收集项目上下文，解析路径。
...
1. Self-Adaptive 初始化 + 项目上下文收集
   ...
   ```
   成功标志：
   - `.skills_local/ic-arch/local.json` 存在
   - `.skills_local/ic-arch/project_context.json` 存在
   - `.skills_local/ic-arch/paths.json` 存在

2. 加载配置文件
   ```python
   import json
   
   with open(".skills_local/ic-arch/project_context.json") as f:
       CONTEXT = json.load(f)
   
   DESIGN_TYPE = CONTEXT.get("design_type", "asic")  # asic, fpga, analog
   TECHNOLOGY = CONTEXT.get("technology", "unknown")  # 28nm, 40nm, etc.
   
   with open(".skills_local/ic-arch/paths.json") as f:
       PATHS = json.load(f)
...
3. 创建执行日志目录
   ```bash
   mkdir -p "${LOG_DIR}"
   LOG_FILE="${LOG_DIR}/ic-arch-$(date -u +%Y%m%dT%H%M%S).log"
...
SCRIPT_DIR        = ~/.claude/scripts
SKILL_FILE        = ~/.claude/skills/ic-arch/SKILL.md
KNOWLEDGE_DIR     = ~/.claude/skills/ic-arch/knowledge
TEMPLATE_DIR      = ~/.claude/skills/ic-arch/templates
```
...
> 以下条件全部满足才可声明 ic-arch 完成，并移交 ic-mas。缺一不可。
