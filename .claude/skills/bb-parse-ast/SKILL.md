---
name: bb-parse-ast
description: "用 pyverilog 解析 SystemVerilog RTL 为 AST JSON，供 CDC 检查 / signal path tracing / module dep 分析。主解析器；失败时切换 bb-parse-ast-fallback。触发场景：(1) bb-guru-synthesis CDC 检查前；(2) bb-trace-signal-path / bb-find-module-deps 之前；(3) 显式 /bb-parse-ast。"
---
...
- 调用者：`bb-guru-synthesis`、`bb-check-cdc`、`bb-trace-signal-path`、`bb-find-module-deps`
