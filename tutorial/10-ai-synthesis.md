...
| DFFASRHQNx1_ASAP7_75t_L | 990 | 异步复位触发器 |
...
| `techmap` | 映射 | 将高级 construct 映射到基本门 |
...
值得注意的是，实际项目中 Agent 将 CLK_SYS 从原始的 500 MHz（2.0ns 周期）放宽到 250 MHz（4.0ns 周期），这是因为初版 RTL 的关键路径延迟过长。Agent 在综合报告中记录了原因："WNS violated by ~1020 ns at 500 MHz, relaxed to 250 MHz to achieve timing closure"。
...
Babel 项目使用 ASAP7（ASU Advanced Performance 7nm）预测性 PDK，这是一个学术级 7nm 工艺库。关键特性：
