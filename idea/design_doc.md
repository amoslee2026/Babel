...
related:
  - idea/decisions.md
  - idea/.review/issues.md
---

# Babel — AI 原生 Chiplet 多 Agent 系统设计方案 (v1.1)

> v1.1 修订说明：根据 `idea/.review/issues.md` 修复 46 项 issue（4 CRITICAL / 15 HIGH / 20 MEDIUM / 7 LOW），同时收纳 3 项用户简化决策（ADR-007 claude-mem / ADR-008 VSCode waveform / ADR-009 EDA = skill only）。关键决策见 `idea/decisions.md`。
...
关键设计决策以 ADR 形式记录在 `idea/decisions.md`：
...
每对 agent 之间的契约由 JSON Schema 强制：

| Upstream | Downstream | Schema |
|----------|------------|--------|
| user | spec-planner | schemas/idea.schema.json |
| spec-planner | rtl-coder | schemas/spec.schema.json |
| rtl-coder | clock-domain-guard | schemas/rtl_artifact.schema.json |
| clock-domain-guard | yosys-synth-planner | schemas/cdc_report.schema.json |
| yosys-synth-planner | test-architect | schemas/synth_report.schema.json |
| any agent | coordinator | schemas/event.schema.json |
| coordinator | (state) | schemas/design_state.schema.json |

启动 hook `babel-hook-validate-input-schema` 校验上游输出；不通过则阻断下游。
...
| 5 | yosys-synth-planner | babel-invoke-yosys | rtl + final sdc | `synth/netlist.v`, `synth/qor.json` | ~500KB |
| 6 | test-architect | babel-generate-tb, babel-invoke-verilator, babel-collect-coverage | rtl + netlist | `tb/uart_tb.sv`, `sim_results/`, `coverage.json` | ~200KB |
...
| 路径 | 用途 |
|------|------|
| `idea/decisions.md` | ADR 决策日志（ADR-001~009，本文档 §0.3 引用） |
| `idea/.review/issues.md` | v1.0 spec-review 评审 issue 清单 |
